...
% i = tsearch(p(:,1),p(:,2),t,pi(:,1),pi(:,2)); for v2009

    % This method for points P in 2D and 3D space
    % DT = DelaunayTri(p)
    % i = pointLocation(DT,p); % for v2012-13 (tsearch removed)

i = tsearchn(p,t,pi); % where p (MxN, m points in n-D space) are the original points, t is the D_triangulation and pi (PxN, p points in n-D space) are the query points
    % the same as original tsearch except that (px,py,t,pix,piy) changed to (p,t,pi) i.e. x,y space given as single matrix rather than two vectors
